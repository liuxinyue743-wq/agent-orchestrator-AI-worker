...
class AOClient:
    ...
    def resolve_approval(
        ...
        """Resolve one pending approval request on an AO Chat session.

        Ported from ao-supervision-sidecar ao_adapter.resolve_approval: used
        by the bounded auto-approval policy (loopcore/approvals.py). Returns
        True only when the daemon accepted the resolution; any HTTP or
        transport failure returns False (never raises) so the caller can keep
        the request pending for a human.
        """

        if not isinstance(request_id, str) or not request_id:
            raise ValueError("request_id must be a non-empty string")
        if decision not in ("allow", "allow_once", "deny"):
            raise ValueError("decision must be allow/allow_once/deny")

        path = (
            f"/api/v1/sessions/{_path_id(session_id, 'session_id')}"
            f"/conversation/approvals/{_path_id(request_id, 'request_id')}"
            "/resolve"
        )
        try:
            response = self._request(
                path, method="POST", json_body={"decisionId": decision}
            )
        except AOClientError:
            return False
...
def _resolve_run_file(
    ...
    # v2 addition: honor AO_DATA_DIR/ao.run before the default home runfile,
    # so installs launched with a custom data dir resolve their own daemon.
    data_dir = os.environ.get("AO_DATA_DIR")
    if data_dir:
        candidate = Path(data_dir).expanduser() / "ao.run"
        if candidate.exists():
            return candidate
